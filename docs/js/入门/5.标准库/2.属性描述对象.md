...
`writable`属性是一个布尔值，决定了**目标属性的值（value）是否可以被改变。**
...
上面这个方法的问题在于，如果遇到存取器定义的属性，会只拷贝值（即只会包括元属性中的value,而不会拷贝其他原属性）。
...
`Object.preventExtensions`方法可以使得一个对象**无法再添加新的属性**。
...
`Object.seal`方法使得一个对象既**无法添加新属性，也无法删除旧属性**。
